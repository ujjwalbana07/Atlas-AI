...
os.environ.setdefault("WEATHER_SERVER_PATH", str(WEATHER_SERVER_PATH))
...
DEFAULT_ORIGIN_CITY = os.getenv("DEFAULT_ORIGIN_CITY", "Dallas, Texas")
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA", "DFW")
...
"""
    ...
    if not allowed:
        reason = guardrail_reason or (
            "Atlas AI can only help with travel-planning requests. "
...
Default origin when the user does not specify one:
{default_origin} ({default_origin_iata})

Use worldwide airport and city data. Do not limit results to one country.
...
def revise_itinerary_agent(state: TravelState):
    feedback = state.get("human_feedback", "").strip()
    prompt = f"""
Revise the draft travel itinerary using the user's feedback below.

User Request:
{state['user_query']}

Current Draft:
{_bounded_text(state.get('itinerary', ''), 9000)}

User Feedback (required changes):
{feedback}

Apply every explicit change in the feedback. Treat numbers, dates, duration,
locations, and budget limits in the feedback as authoritative. For example, if
the user changes a trip from 5 days to 10 days, the revised itinerary must have
10 days and must add the missing days rather than merely mentioning the request.
Return only the complete revised itinerary, not a summary of the changes.
"""
    ...
    return {
        "itinerary": response.content,
        "messages": [AIMessage(content="Draft itinerary revised using human feedback.")],
        "llm_calls": state.get("llm_calls", 0) + 1,
    }
...
Human Review:
{_bounded_text(review_instruction, 1400)}

User Request:
{_bounded_text(state['user_query'], 1600)}

Supervisor Constraints:
{_bounded_text(state.get('trip_constraints', {}), 1200)}

Flights:
{_bounded_text(state.get('flight_results', ''), 1800)}

Hotels:
{_bounded_text(state.get('hotel_results', ''), 1800)}

Weather:
{_bounded_text(state.get('weather_results', ''), 1800)}

Budget Analysis:
{_bounded_text(state.get('budget_results', ''), 1800)}

Draft Itinerary:
{_bounded_text(state.get('itinerary', ''), 5200)}
...
def route_after_approval(state: TravelState) -> str:
    return "final_agent" if state.get("approved", False) else "revise_itinerary"
...
graph.add_node("revise_itinerary", revise_itinerary_agent)
...
graph.add_conditional_edges(
    ...
    {"revise_itinerary": "revise_itinerary", "final_agent": "final_agent"},
)
graph.add_edge("revise_itinerary", "final_agent")
